...
export default {
  ...
  mounted() {
    axios.get(process.env.VUE_APP_API_URL + '/fields/' + this.$route.params.id)
        .then((response)=>{
          let field = response.data.data
          this.form.id = field.id
          this.form.title = field.title
          this.form.type = field.type
          this.form.alias = field.alias
          if(field.extra){
            this.form.extra =field.extra
          }
        })
  },
  ...
  methods:{
    ...
    update(){
      axios.put(process.env.VUE_APP_API_URL + '/fields/' + this.form.id,this.form)
          .then(()=>{
